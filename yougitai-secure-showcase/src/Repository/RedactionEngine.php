...
final class RedactionEngine {
    ...
    public function build_public_content( string $path, string $content, array $rules = [], array $ai_findings = [] ): array {
        ...
        foreach ( $ai_findings as $finding ) {
            ...
            // AI never gets permission to publish sensitive source. Strong recommendations
            // are converted into a conservative whole-file protection action before persistence.
    ...
    public function is_blocked_path( string $path ): bool {
        ...
        if ( preg_match( '/\.(?:pem|p12|pfx|key|keystore|jks)$/i', $basename ) ) {
            return true;
        }
        return (bool) preg_match( '/(^|\/)(private|secrets?|credentials|certificates?)(\/|$)/i', $normalized );
    }

    /** @return array{content:string,visibility:string,risk_score:int,notes:array<int,string>,findings:array<int,array<string,mixed>>} */
