...
final class Renderer {
    ...
    public function template_redirect(): void {
        if ( get_query_var( 'yougitai_showcase_index' ) ) {
            status_header( 200 ); nocache_headers(); $this->register_assets();
            wp_enqueue_style( 'yougitai-ss-frontend' ); wp_enqueue_script( 'yougitai-ss-frontend' );
            get_header();
            ...
            echo '<main class="yougitai-standalone-wrap yougitai-repositories-index"><header class="yougitai-index-header"><h1>' . esc_html( $index_title ) . '</h1><p>' . esc_html( $index_description ) . '</p></header>' . $this->render_grid( 'auto', 3 ) . '</main>';
            get_footer(); exit;
        }
        ...
        wp_enqueue_style( 'yougitai-ss-frontend' ); wp_enqueue_script( 'yougitai-ss-frontend' );
        get_header();
        echo '<main class="yougitai-standalone-wrap">' . $this->render( [ 'id' => (int) $repository['id'], 'theme' => 'auto' ] ) . '</main>';
        get_footer(); exit;
    }

    public function shortcode( array $atts = [] ): string {
        $atts = shortcode_atts( [ 'id' => 0, 'slug' => '', 'theme' => 'auto' ], $atts, 'yougitai_showcase' );
        return $this->render( [ 'id' => absint( $atts['id'] ), 'slug' => sanitize_title( $atts['slug'] ), 'theme' => $this->sanitize_theme( (string) $atts['theme'] ) ] );
    }
    ...
    public function render( array $args ): string {
        ...
        if ( $selected ) $selected = $this->decorate_public_file( $selected );
        ...
        ob_start(); include YOUGITAI_SS_DIR . 'templates/showcase.php'; return (string) ob_get_clean();
    }
    ...
    public function decorate_public_file( array $file ): array {
        ...
        if ( ! empty( $file['redaction_manifest'] ) ) {
            ...
            if ( is_array( $decoded ) ) $manifest = array_values( array_filter( $decoded, 'is_array' ) );
        }
        if ( ! $manifest && ( $file['visibility'] ?? '' ) === 'redacted' && isset( $file['content'] ) ) {
            ...
            foreach ( [ '[REDACTED]', '[PROTECTED]', '[PROTECTED BY SECURITY REVIEW]' ] as $token ) {
                ...
                while ( ( $pos = strpos( $content, $token, $offset ) ) !== false ) {
                    ...
                    $last = strrpos( $prefix, "\n" );
                    $manifest[] = [ 'kind'=>'legacy','line_start'=>$line,'line_end'=>$line,'column_start'=>max(1,$pos-( $last===false ? -1 : $last )),'original_length'=>strlen($token),'replacement'=>$token ];
        ...
        foreach ( $manifest as $entry ) {
            $start = max( 1, (int) ( $entry['line_start'] ?? 1 ) ); $end = max( $start, (int) ( $entry['line_end'] ?? $start ) );
            for ( $line = $start; $line <= $end; $line++ ) $lines[ $line ] = true;
        }
    ...
    private function build_file_tree( array $files ): array {
        ...
        foreach ( $files as $file ) {
            ...
            foreach ( $parts as $index => $part ) {
                ...
                if ( $is_file ) { $cursor['__files'][] = $file; continue; }
    ...
    private function sanitize_theme( string $theme ): string { return in_array( $theme, [ 'auto', 'light', 'dark' ], true ) ? $theme : 'auto'; }

    private function statistics( array $files ): array {
        $languages = []; $size = 0; $redacted = 0;
        foreach ( $files as $file ) {
            $language = (string) ( $file['language'] ?: __( 'Other', 'yougitai-secure-showcase' ) );
            $languages[ $language ] = ( $languages[ $language ] ?? 0 ) + 1; $size += (int) $file['size'];
